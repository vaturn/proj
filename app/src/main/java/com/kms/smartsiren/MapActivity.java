...
public class MapActivity extends AppCompatActivity {
            ...
            public void onSuccess(Location location) {
                if (location != null) {
                    ...
                    centerLabel = labelLayer.addLabel(LabelOptions.from("centerLabel", LatLng.from(location.getLatitude(), location.getLongitude()))
                            .setStyles(LabelStyle.from(R.drawable.usercurrentlocation).setAnchorPoint(0.5f, 1.0f))
                            .setRank(1));
                }
            }
        });
    ...
    //검색창에 입력한 주소문자열 받아오고 해당 주소를 위도,경도로 변환해서 해당 위치로 카메라를 조정
